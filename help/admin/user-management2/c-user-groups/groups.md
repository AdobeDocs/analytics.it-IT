---
description: Descrizioni degli elementi nella pagina Definisci gruppo di utenti.
seo-description: Descrizioni degli elementi nella pagina Definisci gruppo di utenti.
seo-title: Impostazioni gruppo utenti
solution: Analytics
subtopic: Utenti e gruppi
title: Impostazioni gruppo utenti
topic: Strumenti di amministrazione
uuid: b411032a-7d56-44fd-b676-abee4af756f9
translation-type: tm+mt
source-git-commit: 45e3330adb562ec795d287ae1c1fa6b03a2b2a31

---


# Impostazioni gruppo utenti (legacy)

>[!IMPORTANT]
>
>La gestione di utenti e prodotti passa ad [Admin Console](https://helpx.adobe.com/enterprise/using/admin-console.html). Adobe ti informerà quando è il momento di eseguire la migrazione degli utenti. Dopo la migrazione di tutti i clienti, i contenuti della guida per **[!UICONTROL Analytics]** &gt; **[!UICONTROL Admin Tools]** &gt; **[!UICONTROL User Management]** verranno ritirati.

Descrizioni degli elementi nella pagina Definisci gruppo di utenti.

**[!UICONTROL Admin]** &gt; **[!UICONTROL User Management]** &gt; **[!UICONTROL Groups]** &gt; **[!UICONTROL Add New User Group]**

<table id="table_85E9EBF7984545C780631D6C2090BD99"> 
 <thead> 
  <tr> 
   <th colname="col1" class="entry"> Elemento </th> 
   <th colname="col2" class="entry"> Descrizione </th> 
  </tr> 
 </thead>
 <tbody> 
  <tr> 
   <td colname="col1"> <p>Nome gruppo </p> </td> 
   <td colname="col2"> <p>Nome del gruppo. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Descrizione gruppo </p> </td> 
   <td colname="col2"> <p>Una descrizione dello scopo del gruppo, ad esempio il tipo di autorizzazioni che concede. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><b>Accesso alla suite di rapporti</b> </p> </td> 
   <td colname="col2"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_62D4A028A1194E02B2F0D364D01E741C"> 
      <li id="li_11ADAD0A625046409B4FCCEF1D14E1E6">Suite di rapporti disponibili </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>Visualizza tutte le suite di rapporti a cui il gruppo ha accesso. </p> <p>Selezionate le suite di rapporti desiderate, quindi fate clic su <span class="uicontrol"> Aggiungi</span>. </p> <p>Le suite di rapporti assegnate vengono visualizzate nelle <span class="wintitle"> suite di rapporti incluse nel gruppo</span>. </p> <p><b>Accesso</b>a tutte le suite di rapporti: Quando aggiungete delle suite di rapporti da includere in un gruppo, potete specificare <span class="uicontrol"> Accesso</span>a tutte le suite di rapporti. Questa impostazione applica le autorizzazioni del gruppo a tutte le suite di rapporti correnti e future. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_2615D113681C402E8F6B2A844B402626"> 
      <li id="li_6E04FC6AE26F4052835EF3AE51CDE2E3">Suite di rapporti incluse nel gruppo </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>Visualizza tutte le suite di rapporti attualmente assegnate al gruppo. </p> <p>Per rimuovere una suite di rapporti, selezionatela, quindi fate clic su <span class="uicontrol"> Rimuovi</span>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><b>Accesso ai rapporti</b> </p> </td> 
   <td colname="col2"> 
    <draft-comment> 
     <p> 
      <ul id="ul_81ABB1701BEC44ECB548F98EB818F83B"> 
       <li id="li_FCE10A56AF904C9CBCE24AC91025639C"><b>Rapporti</b>sul traffico: Consente l'accesso a tutti i rapporti in base al traffico, come visualizzazioni di pagina, visite e visitatori unici. </li> 
       <li id="li_2742B7A661FC45F496DFFBBB6CE675DE"><b>Strumenti</b> (consente l’accesso agli strumenti utilizzati in Reporting e analisi, ad esempio Escludi per IP, Classificazioni e ClickMap legacy). </li> 
       <li id="li_88D54C31211B4A38B46FF64651718865"><b>Rapporti video e mobili</b> </li> 
       <li id="li_DBC756832B52433DA6467EA8AEC02DBA"><b>Rapporti</b> percorso (consente l’accesso ai rapporti sui percorsi come Pagina successiva, Abbandono e Tempo trascorso sulla pagina). </li> 
       <li id="li_D0B1B964C071441EAEC919DD7CC08E3D"><b>Report percorso personalizzato</b> </li> 
       <li id="li_D63F60FF2DD2418A876B53404634D12D"><b>Test e Target </b>(concede l'accesso ai report di Target se l'azienda utilizza sia Analytics che Target). </li> 
       <li id="li_03CE1718D51049D596DB061932D97506"><b>SearchCenter</b> </li> 
       <li id="li_78AB0BCEDBA946718F90B6D7AB52CB80"><b>Servizi Web </b>(consente l'accesso alle risorse API per reporting e amministrazione.) </li> 
       <li id="li_DCA83780F6214AF1A82255BC977744F1"><b>Metriche</b> (consente l'accesso alle metriche utilizzate in Reporting e analisi, comprese le metriche predefinite come le visualizzazioni di pagina e altre metriche come gli eventi personalizzati).</li> 
       <li id="li_A200E3CED33D4B15BFD30CBDD930912C"><b>Admin Console </b>(consente l’accesso a vari componenti nel menu Admin, come le impostazioni Registri e Suite di rapporti). </li> 
       <li id="li_CF3FA04D402A47618401633BA8583010"><b>Download</b> dei report (consente di scaricare i report dall'interfaccia utente di Analytics). </li> 
       <li id="li_03AC2D9FF43648CB90E514C62DC31217"><b>Sondaggio</b> (consente l’accesso ai rapporti del sondaggio se l’organizzazione utilizza anche la funzionalità Adobe Survey obsoleta). </li> 
       <li id="li_9FC3F25F335641B5AC9232BDC2DCCECA"><b>Generazione di rapporti</b> avanzati (consente l'accesso a rapporti avanzati come canali di marketing, rilevamento delle anomalie, segmenti e metriche calcolate). </li> 
       <li id="li_DB56BEB8D31A4918B5CCD3CBBC1DF40A"><b>Social</b> (consente l'accesso ai report generati dai dati in Adobe Social). </li> 
       <li id="li_3C2F4F91B92A4CD9BCDD69E6FCE85EEE"><b>Connettori dati </b>(Consente agli utenti di creare, aggiornare o eliminare connettori dati.) </li> 
      </ul> </p> 
    </draft-comment> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_CE3DC9C5D63348E48D72BEC32C9A2B45"> 
      <li id="li_9636E0FA37BE4FE48F8723F46C96FFC4">Tutti gli accessi </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>I membri del gruppo hanno accesso a tutti gli elementi del gruppo di rapporti. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_B573C170710545F0BF5CC3675A8F7ECA"> 
      <li id="li_238BA42873E24953A0D73443F36BD7C8">Personalizza </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>I membri del gruppo possono accedere a un sottoinsieme dei rapporti nel gruppo di report. Fate clic su <span class="uicontrol"> Personalizza</span> per aprire una pagina in cui potete selezionare i rapporti specifici nel gruppo di rapporti a cui i membri del gruppo possono accedere. </p> 
    <ul id="ul_16D5EF18D57D4608AEEDEC40D90D8828"> 
     <li id="li_F29E84C6228A464C8807F09205AEAAC6"><a href="/help/admin/user-management2/c-customize-report-access/groups-analytics-tools.md" format="dita" scope="local"> Strumenti</a>di Analytics: Attiva le autorizzazioni utente per gli elementi generali (fatturazione, registri ecc.), gestione società, strumenti, accesso ai servizi Web, Generatore di report e integrazione dei connettori dati. </li> 
     <li id="li_A6EB788162A2455E94CE54B9279A854D"><a href="/help/admin/user-management2/c-customize-report-access/groups-report-suite-tools.md" format="dita" scope="local"> Strumenti</a>suite di rapporti: Abilita le autorizzazioni degli utenti per Accesso API, Gestione suite di rapporti, Strumenti e Rapporti ed Elementi dashboard. </li> 
     <li id="li_EDB0255E009B4F1CAFAF53966B41363C"><a href="/help/admin/user-management2/c-customize-report-access/groups-metrics.md" format="dita" scope="local"> Metriche</a>: Attiva le autorizzazioni per il traffico, la conversione, gli eventi personalizzati, gli eventi delle soluzioni, in base al contenuto e così via. </li> 
     <li id="li_8DAE87D1DEF54803A9C6FE31C01F0FB0"><a href="/help/admin/user-management2/c-customize-report-access/groups-dimensions.md" format="dita" scope="local"> Dimensioni</a>: Personalizza l'accesso degli utenti a un livello granulare, comprese eVar, rapporti sul traffico, rapporti sulle soluzioni e rapporti sui percorsi. </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_F992DC52D44B4E1E9448D699A57AA7A9"> 
      <li id="li_5D07359AF741418FB73468400686CB57">Nessun accesso </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>I membri del gruppo non possono accedere ai rapporti nel gruppo di rapporti. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"><b>Tutte le suite di rapporti</b> </td> 
   <td colname="col2"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_19FCA8B24B7E4477B9AC9564D6CF2D44"> 
      <li id="li_F006EF42026F43069AEC0BFE01D7CBC8">TagManager (Accesso completo, Personalizzazione, Nessun accesso) </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>Le autorizzazioni sono supportate per i clienti che dispongono di contenitori <span class="keyword"> TagManager</span> attivi. Non disponibile per i nuovi utenti. </p> <p>Adobe consiglia <a href="https://marketing.adobe.com/resources/help/en_US/dtm/atm_migration.html" format="html" scope="external"> Gestione</a> tag dinamica per la distribuzione. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"><b>Assegna login utente</b> </td> 
   <td colname="col2"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_8C68B33A3847467DBDC58838E3E85612"> 
      <li id="li_9BA51B0001AA41DE949322083A6CE70D">Utenti disponibili </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>Visualizza tutti gli account utente attualmente definiti. Selezionate gli account utente desiderati, quindi fate clic su <span class="uicontrol"> Aggiungi</span>. Gli account utente assegnati vengono visualizzati nel campo Membri <span class="uicontrol"> del</span> gruppo. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 
     <ul id="ul_5EAE5B2B54EA4C87BAA0C30AE9C8ECB0"> 
      <li id="li_FB6C9E925A5E498BBAFE13EF7606E940">Membri gruppo </li> 
     </ul> </p> </td> 
   <td colname="col2"> <p>Visualizza tutti gli account utente attualmente assegnati come membri del gruppo. Per rimuovere un account utente, selezionatelo, quindi fate clic su <span class="uicontrol"> Rimuovi</span>. </p> </td> 
  </tr> 
 </tbody> 
</table>

