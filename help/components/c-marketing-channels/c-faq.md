...
* Nel sistema e-mail, aggiungi un parametro della stringa di query a tutti gli URL della pagina di destinazione, ad esempio *`&ch=eml`*. Crea un set di regole rilevando se il parametro di query ch è uguale a *`eml`*. Se non contiene *`eml`* allora è un affiliato.
...
Hai una regola che corrisponde al traffico interno. Tieni presente che queste regole elaborano per ogni hit che un visitatore fa sul tuo sito, non solo per la prima visita. Se hai una regola come *`Page URL exists`* senza altri criteri, quel canale corrisponde a ogni hit successivo sul sito, perché esiste sempre un URL di pagina.
...
1. **[!UICONTROL Referrer]** e **[!UICONTROL Does Not Exist]** e **[!UICONTROL Is First Page of Visit]**. (Vedi [Diretta](/help/components/c-marketing-channels/c-faq.md))

2. **[!UICONTROL Referrer Matches Internal URL Filters]** e **[!UICONTROL Is First page of Visit]**. (Vedi [Interno](/help/components/c-marketing-channels/c-faq.md).)
...
Quando le regole non acquisiscono dati o se non sono configurate correttamente, il rapporto visualizza i dati nel [!UICONTROL No Channel Identified] sul report. Puoi creare un set di regole denominato *Altro*, ad esempio, al termine dell’ordine di elaborazione, che identifica anche il traffico interno.

![](assets/example_other.png)

Questo tipo di regola funge da elemento catch-all per garantire che il traffico del canale corrisponda sempre al traffico esterno e in genere non finisca in **[!UICONTROL No Channel Identified]**. Fai attenzione a non creare una regola che identifichi anche il traffico interno. Impostazione del valore del canale su **[!UICONTROL Referring Domain]** o **[!UICONTROL Page URL]** sono i modi più comuni e utili per creare un&#39;altra regola efficace.
...
* **Traffico tra domini**: Un visitatore si sposta da un dominio che viene attivato nella suite A a un secondo dominio che viene attivato nella suite B. Se nella suite B i filtri URL interni includono il primo dominio, la visita nella suite B verrà registrata come interna, poiché i canali di marketing la vedono come una nuova visita nella seconda suite. La visita verrà classificata come Aggiornamento sessione.
...
Di seguito è riportato un esempio di impostazione di Interno (aggiornamento sessione) sia come canale di primo e ultimo contatto:
...
* **Scadere manualmente tutti i periodi di coinvolgimento del visitatore**: Questa impostazione scade immediatamente tutti i canali di primo e ultimo contatto tra tutti i visitatori:
...
* **Visualizza solo le metriche Last Touch dal momento in cui hai corretto le regole in avanti**: Le metriche Last Touch seguono sempre il set di regole corrente. La visualizzazione del tempo da cui le regole di elaborazione sono state modificate riflette correttamente le regole di elaborazione più recenti.
