...
La sostituzione dei tag con più suite con una suite di rapporti globale e una VRS consente di semplificare l&#39;implementazione Adobe  Analytics e di ridurre il consumo di chiamate server, ed è consigliato come procedura ottimale. Tuttavia, esistono alcune importanti limitazioni della VRS da considerare. Le seguenti linee guida possono aiutarti a decidere se implementare le suite di rapporti virtuali basate su una suite di rapporti globale sia l&#39;approccio giusto per te.

## Linee guida

Se non siete sicuri se i casi di utilizzo descritti si applicano a voi e alla vostra organizzazione, rivolgetevi agli altri amministratori di Adobe  Analytics o al vostro account manager Adobe. Possono aiutare a valutare le esigenze aziendali e a formulare una raccomandazione.
...
La condivisione di segmenti in Adobe Experience Cloud non è supportata per le suite di rapporti virtuali. Gli utenti che desiderano condividere un segmento con l&#39;Experience Cloud  devono avere accesso alla suite di rapporti di origine.
...
Le origini dati di riepilogo consentono di importare metriche aggregate in Adobe  Analytics a livello di suite di rapporti. Poiché i caricamenti delle origini dati di riepilogo contengono metriche aggregate, non possono essere segmentati. Poiché la VRS funziona utilizzando la segmentazione, tutti i dati importati utilizzando origini dati di riepilogo non sono disponibili nelle suite di rapporti virtuali. Le origini dati di riepilogo sono visibili solo nella suite di rapporti di origine.
