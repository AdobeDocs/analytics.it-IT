...
Alcuni fattori possono influenzare le prestazioni di un progetto in Analysis Workspace. È importante sapere quali sono questi contributi prima di iniziare a creare un progetto in modo da poter pianificare e creare il progetto nel modo migliore. Di seguito è riportato un elenco di fattori che influiranno sulle prestazioni e sulle tecniche consigliate per l’ottimizzazione dei tuoi progetti. Le prestazioni di Analysis Workspace sono una delle priorità principali di Adobe e continuiamo a migliorare ogni giorno.

## Complessità della logica del segmento

Segmenti complessi possono avere un impatto significativo sulle prestazioni del progetto. Fattori che aggiungono complessità a un segmento (in ordine decrescente di impatto) includono:

* Operatori di "contains", "contains any of", "matches", "start with" o "ends with"
...
* Contenitori nidificati che variano nell’ambito (ad esempio, "Hit" all’interno di "Visit" all’interno di "Visitor")
...
* Con gli operatori, "uguale" sarà più veloce di "contiene" e "uguale a uno qualsiasi di" sarà più veloce di "contiene uno qualsiasi di".
* Con molti criteri, gli operatori AND saranno più veloci di una serie di operatori OR. Inoltre, cercare le opportunità per ridurre molte istruzioni OR in un'unica istruzione "uguale a qualsiasi".

Inoltre, l’uso delle [classificazioni](/help/components/c-classifications2/c-classifications.md) può aiutare a consolidare più valori in gruppi compatti, dai quali è possibile creare dei segmenti. La segmentazione sui gruppi di classificazioni offre vantaggi in termini di prestazioni rispetto ai segmenti che contengono molte istruzioni OR o criteri "contiene".
...
**Best practice per l'intervallo di dati**

Laddove possibile, non inserire più dati del necessario.
...
**Procedure ottimali per il numero di visualizzazioni**
...
**Best practice per la complessità della visualizzazione**
...
**Procedure ottimali per il numero di pannelli**

Non cercare di aggiungere tutto in un progetto, ma piuttosto di creare progetti distinti che servano uno scopo specifico o un gruppo di soggetti interessati. Utilizza i tag per organizzare i progetti in temi chiave e condividi i progetti correlati con i soggetti interessati.
...
Se gli utenti accedono a suite di rapporti diverse, il numero di utenti che accedono contemporaneamente ad Analysis Workspace o a progetti specifici non influisce in modo sostanziale sulle prestazioni di Analysis Workspace. Se gli utenti simultanei accedono alla stessa suite di rapporti, le prestazioni risulteranno compromesse.

## Messaggi di errore comuni in Analysis Workspace

È possibile che si verifichino errori durante l’interazione con Analysis Workspace. Gli errori possono verificarsi per diversi motivi ed elencati di seguito sono i più comuni.

| Messaggio di errore | Perché questo accade? |
|---|---|
| `The report suite is experiencing unusually heavy reporting. Please try again later.` | L'organizzazione sta tentando di eseguire troppe richieste simultanee su una suite di rapporti specifica. I collaboratori di questo errore sono richieste API, progetti pianificati, rapporti pianificati, avvisi pianificati e utenti simultanei che eseguono richieste di reporting. È consigliabile che le richieste e le pianificazioni per la suite di rapporti siano distribuite in modo più uniforme nel corso della giornata. |
| `A system error has occurred. Please log a Customer Care request under Help > Submit Support Ticket and include your error code.` | Adobe sta riscontrando un problema da risolvere. È consigliabile inviare il codice di errore tramite una richiesta dell'Assistenza clienti. |
| `The request is too complex.` | La richiesta di reporting è troppo grande e non può essere eseguita. I collaboratori a questo errore sono timeout dovuti alla dimensione della richiesta, troppi elementi associati in un segmento o un filtro di ricerca, troppe metriche incluse, combinazioni di dimensioni e metriche incompatibili, ecc. È consigliabile semplificare la richiesta. |
| `One of the segments or the search in this visualization contains a text search that returned too many results.` | È consigliabile restringere i criteri di testo di ricerca e riprovare. |
| `This dimension does not currently support non-default attribution models.` | È consigliabile sostituire la dimensione nella tabella con una che sia compatibile con [Attribution IQ](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/panels/attribution.html). |
| `Your request failed as a result of too many columns or pre-configured rows.` | È consigliabile rimuovere alcune colonne o righe oppure suddividerle in visualizzazioni separate. |
