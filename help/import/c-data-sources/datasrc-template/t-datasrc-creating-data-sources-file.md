...
1. Il giorno **[!UICONTROL Data Sources Create]** , selezionare una categoria e digitare un modello.
1. Rivedi le istruzioni/informazioni di attivazione, quindi fai clic su **[!UICONTROL Activate]**.

   Risultato passaggio 1. Seleziona le opzioni di generazione modello nella procedura guidata Attivazione origine dati.

   | Pagina della procedura guidata | Campo | Descrizione |
   |--- |--- |--- |
   | 1 | Nome | Il nome del modello visualizzato da Analytics in Gestione origini dati. |
   | 1 | E-mail | L&#39;indirizzo e-mail che riceve tutte le notifiche correlate all&#39;utilizzo di questo modello Origine dati. |
   | 1 | Casella di controllo Associated Fees (Spese associate) | Selezionare la casella di controllo per indicare l&#39;accettazione delle tariffe associate all&#39;utilizzo di questo modello di origine dati. |
   | 2 | Choose Metrics (Scegli metrica) | Seleziona la metrica da importare utilizzando questa Origine dati. Analytics consiglia determinate metriche in base alla categoria e al tipo di origine dati selezionati nel passaggio 3.  Per specificare una metrica diversa, digita il nome in un campo vuoto e seleziona la casella di controllo per attivare la metrica. |
   | 3 | Map Metrics (Mappa metrica) | Seleziona un evento di Analytics per ricevere ogni metrica importata selezionata nella pagina 2 della procedura guidata.  Devono essere eventi nuovi e non assegnati a cui in precedenza sono stati assegnati nomi corrispondenti ai dati della metrica importati che riceveranno tramite Origine Dati.  Se una variabile eVar, Prodotto o Codice di tracciamento è una variabile di destinazione e i valori caricati corrispondono ai valori acquisiti esistenti, gli eventi caricati aggiungono fondamentalmente le metriche ai valori esistenti. Ad esempio, puoi creare una metrica &quot;Ordini offline&quot; con una dimensione dati Prodotti che presenta già come metriche esistenti Viste prodotto, Pagamenti e Ordini. |
   | 4 | Choose Data Dimensions (Scegli dimensioni dati) | Seleziona le dimensioni dei dati per suddividere le metriche importate da questa Origine dati. Analytics consiglia determinate dimensioni dati in base al tipo di origine dati selezionato nel passaggio 3.  Per specificare una dimensione dati diversa, digita il nome in un campo vuoto e seleziona la casella di controllo per attivare la dimensione dati. |
...
1. Dopo la generazione del modello, copia i dati nelle colonne opportune del modello Origine dati, verificando di rispettare il formato dati richiesto per tale colonna di dati.
   ...
   Risultato passaggio 1. Salva il file Origini dati utilizzando una convenzione di denominazione prescelta. Adobe consiglia di utilizzare una convenzione di denominazione coerente per tutti i file Origini dati. Utilizza un’estensione di file comune, ad esempio .txt o .tsv (oppure non utilizzare alcuna estensione).
