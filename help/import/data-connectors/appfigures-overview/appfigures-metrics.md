---
description: Le metriche seguenti sono sincronizzate nell'ambito dell'integrazione appFigures.
seo-description: Le metriche seguenti sono sincronizzate nell'ambito dell'integrazione appFigures.
...
Le metriche seguenti sono sincronizzate nell'ambito dell'integrazione appFigures.

| Metrica | Descrizione |
|---|---|
| Download dall'app store | Numero di download di applicazioni mobili. |
| Acquisti app store (nell'app) | Il numero di acquisti in-app. |
| Valutazione app store | Utilizzato per definire la metrica calcolata della metrica appFigures media. Non utilizzato direttamente. |
| Divisore classificazione app store | Utilizzato per definire la metrica calcolata della metrica appFigures media. Non utilizzato direttamente. |
| Valutazione app store | Utilizzato per definire la metrica calcolata della metrica appFigures media. Non utilizzato direttamente. |
| Divisore valutazione app store | Utilizzato per definire la metrica calcolata della metrica appFigures media. Non utilizzato direttamente. |
| Ricavi da app store (in-app) | Quantità di ricavi in app. |
| Ricavi da app store (una tantum) | Quantità di ricavi associati all'acquisto di un'app. |
| Royalty dell'app store (nell'app) | Obsoleto. |
| Royalty dell'app store (una tantum) | Obsoleto. |

Le seguenti dimensioni di classificazione riservate sono create dalla procedura guidata di connettori dati per l'ID oggetto AppFigures riservato eVar.

| Dimensione classificazione | Descrizione |
|---|---|
| Nome Paese | Nome del paese correlato all'attività dell'app store. |
| Nome del dispositivo | Nome del dispositivo mobile correlato all'attività dell'app store. |
| Nome Appstore | Nome dell'appstore che ha segnalato l'attività. |
| Nome applicazione | Nome dell’applicazione mobile. |
...
| Commento revisione | Commento del riesame |
| Versione applicazione | Versione dell'applicazione utilizzata per generare la revisione. |
| Utente App Store | Autore (nome utente) della revisione. |

La metrica calcolata seguente viene creata dalla procedura guidata dei connettori dati:

| Metrica calcolata | Descrizione |
|--- |--- |
| Valutazione media app store (numerica) | Classificazione media dell'app. Questa metrica calcolata viene definita utilizzando la seguente formula:   `[App Store Rank]` / `[App Store Rank Divisor]` <br> Nota:  Il numero di applicazioni classificate per categoria e paese varia a seconda degli app store. Se l'applicazione è classificata al di sotto di questa soglia, i ranghi dell'applicazione non sono disponibili. |
| Valutazione media in app store | Valutazione media dell'app. Questa metrica calcolata viene definita utilizzando la seguente formula:  `[App Store Rating]` / `[App Store Rating Divisor]` |

## Dimensioni rilevanti per ogni metrica AppFigures {#section-cd356d3dce04412893beed345305c247}

Nella tabella seguente sono illustrate le dimensioni del rapporto relative alle metriche.
...
<table id="table_B9CF57EABE22449FBF1963E3F105E702"> 
 <tbody> 
  <tr> 
   ...
   <td> Divisore classificazione app store </td> 
   <td> Valutazione app store </td> 
   <td> Divisore valutazione app store </td> 
   <td> Ricavi da app store (in-app) </td> 
   <td> Ricavi da app store (una tantum) </td> 
   <td> Media app store Classifica </td> 
   <td> Media app store Valutazione </td> 
  </tr> 
  <tr> 
   <td> Nome Paese </td> 
  ...
  <tr> 
   <td> Nome categoria classificazione </td> 
