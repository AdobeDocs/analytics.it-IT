...
* **Variabili Adobe Analytics disponibili e configurate:** Questa integrazione richiede eventi personalizzati e evar personalizzati. Consultate [Variabili integrazione di Adobe Analytics](../../optivo-overview/optivo-requirements/optivo-variables.md#concept-8ebd2bde4a1c4b0aad2987e050ffbbfc).
...
* **ID messaggio:** L'integrazione richiede l'acquisizione e l'archiviazione di un «ID messaggio» all'interno di una variabile di Adobe Analytics (evar). Questi ID sono necessari per identificare i collegamenti inviati. Come parte del processo di configurazione, quando richiesto dalla procedura guidata devi identificare un evar a questo scopo.
...
* **Ora clic post:** Come parte del processo di configurazione, questa integrazione richiede un'assegnazione a un evar che corrisponde all'ora di un'azione click clic. Questo è necessario per trasmettere a [!DNL ~Partner informazioni su un'azione del destinatario~] dopo che il destinatario ha fatto clic su un collegamento in una mailing list.

* **Prodotto post-clic:** Come parte del processo di configurazione, questa integrazione richiede un'assegnazione a un evar corrispondente al prodotto offerto associato a un'azione click click. Questo è necessario per trasmettere a [!DNL ~Partner informazioni su un'azione del destinatario~] dopo che il destinatario ha fatto clic su un collegamento in una mailing list.

* **Post Click Type of Action:** Come parte del processo di configurazione, questa integrazione richiede un'assegnazione a un evar corrispondente al tipo di azione click click. Questo è necessario per trasmettere a [!DNL ~Partner informazioni su un'azione del destinatario~] dopo che il destinatario ha fatto clic su un collegamento in una mailing list.
