...
* **Profilo di abbandono del modulo (facoltativo):** Incoraggiare i visitatori a completare un modulo non completato tramite campagne progettate appositamente per coloro che hanno iniziato e non hanno completato un modulo online. Questo segmento verrà creato solo se vengono creati 2 eventi per l'avvio del modulo e il completamento del modulo.
* **Profilo di download file (facoltativo):** Raggiungi potenziali clienti tramite campagne progettate per coloro che hanno mostrato interesse scaricando un white paper o un altro tipo di download. Questo segmento viene creato solo se viene creato un evento 1 per i download dei file riusciti.
