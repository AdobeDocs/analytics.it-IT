...
| **`adload`** | Caricamenti degli annunci multimediali | varchar(255) |
...
| **`browser`** | ID numerico del browser. Fa riferimento a `browser.tsv` tabella di ricerca. | int senza segno |
...
| **`color`** | ID di profondità colore in base al valore del `c_color` colonna. Fa riferimento a `color_depth.tsv` tabella di ricerca. | piccolo senza segno |
| **`connection_type`** | ID numerico che rappresenta il tipo di connessione. Variabile utilizzata nella variabile [Tipo di connessione](/help/components/dimensions/connection-type.md) dimensione. Fa riferimento a `connection_type.tsv` tabella di ricerca. | tinyint senza segno |
| **`cookies`** | Variabile utilizzata nella variabile [Supporto per cookie](/help/components/dimensions/cookie-support.md) dimensione.<br>Y: Abilitato<br>N: Disabilitato<br>U: Sconosciuto | char(1) |
| **`country`** | ID numerico che rappresenta i valori presenti nella variabile `country.tsv` ricerca. Utilizzato nel rapporto sui domini di primo livello in Reports &amp; Analytics. | piccolo senza segno |
| **`ct_connect_type`** | Correlati al `connection_type` colonna. I valori più comuni sono LAN/Wifi, Mobile Carrier e Modem. | char(20) |
...
| **`event_list`** | Elenco degli ID numerici separati da virgole che rappresentano gli eventi attivati sull&#39;hit. Include eventi predefiniti ed eventi personalizzati da 1 a 1000. Usi `event.tsv` ricerca. | text |
| **`exclude_hit`** | Flag che indica che l’hit è escluso dal rapporto. La `visit_num` la colonna non viene incrementata per gli hit esclusi.<br>1: Non utilizzato. Parte di una feature di scarto.<br>2: Non utilizzato. Parte di una feature di scarto.<br>3: Non più utilizzato. Esclusione dell’agente utente<br>4. Esclusione basata sull’indirizzo IP<br>5: Informazioni hit vitali mancanti, ad esempio `page_url`, `pagename`, `page_event`oppure `event_list`<br>6. JavaScript non elaborava correttamente gli hit<br>7: Esclusione specifica dell’account, ad esempio in una regola VISTA<br>8: Non utilizzato. Esclusione alternativa specifica per l’account.<br>9: Non utilizzato. Parte di una feature di scarto.<br>10. Codice valuta non valido<br>11. Hit mancante di una marca temporale in una suite di rapporti solo di marca temporale o un hit contenente una marca temporale in una suite di rapporti non di marca temporale<br>12. Non utilizzato. Parte di una feature di scarto.<br>13. Non utilizzato. Parte di una feature di scarto.<br>14. Hit di Target che non corrisponde a un hit di Analytics<br>15. Non attualmente utilizzato.<br>16. Hit Advertising Cloud che non corrisponde a un hit di Analytics | tinyint senza segno |
| **`first_hit_page_url`** | Il primo URL del visitatore. | varchar(255) |
| **`first_hit_pagename`** | Variabile utilizzata nella variabile [Pagina di ingresso originale](/help/components/dimensions/entry-dimensions.md) dimensione. Il nome originale della pagina di ingresso del visitatore. | varchar(100) |
| **`first_hit_ref_domain`** | Variabile utilizzata nella variabile [Dominio di riferimento originale](/help/components/dimensions/original-referring-domain.md) dimensione. In base a `first_hit_referrer`. Il primo dominio di riferimento del visitatore. | varchar(100) |
| **`first_hit_ref_type`** | ID numerico che rappresenta il tipo di referente del primo referente del visitatore. Usi `referrer_type.tsv` ricerca. | tinyint senza segno |
...
| **`geo_city`** | Nome della città da cui proviene l’hit, in base all’IP. Utilizzato in [Città](/help/components/dimensions/cities.md) dimensione. | char(32) |
| **`geo_country`** | Abbreviazione del paese da cui proviene l&#39;hit, basata su IP. Utilizzato in [Paesi](/help/components/dimensions/countries.md) dimensione. | char(4) |
| **`geo_dma`** | ID numerico dell’area demografica di origine dell’hit, basato su IP. Utilizzato in [DMA USA](/help/components/dimensions/us-dma.md) dimensione. | int senza segno |
| **`geo_region`** | Nome dello stato o della regione da cui proviene l’hit, in base all’IP. Utilizzato in [Aree geografiche](/help/components/dimensions/regions.md) dimensione. | char(32) |
| **`geo_zip`** | Il codice postale di origine dell&#39;hit, basato su IP. Consente di popolare i [Codice postale](/help/components/dimensions/zip-code.md) dimensione. Consulta anche `zip`. | varchar(16) |
...
| **`language`** | ID numerico della lingua. Usi `languages.tsv` tabella di ricerca. | piccolo senza segno |
| **`last_hit_time_gmt`** | Timestamp (in tempo Unix) dell’hit precedente. Utilizzato per calcolare il [Giorni dall’ultima visita](/help/components/dimensions/days-since-last-visit.md) dimensione. | int |
| **`last_purchase_num`** | Variabile utilizzata nella variabile [Fedeltà del cliente](/help/components/dimensions/customer-loyalty.md) dimensione. Il numero di acquisti precedenti effettuati dal visitatore. <br>0: Nessun acquisto precedente (non cliente) <br>1: 1 acquisto precedente (nuovo cliente) <br>2: 2 acquisti precedenti (cliente reso) <br>3: 3 o più acquisti precedenti (cliente leale) | int senza segno |
| **`last_purchase_time_gmt`** | Utilizzato in [Giorni dall’ultimo acquisto](/help/components/dimensions/days-since-last-purchase.md) dimensione. Timestamp (in tempo Unix) dell’ultimo acquisto effettuato. Per gli acquisti e i visitatori che non hanno effettuato un acquisto in precedenza, questo valore è `0`. | int |
...
| **`mobileosenvironment`** | Raccolte dalla variabile di dati di contesto `a.OSEnvironment`. Ambiente del sistema operativo degli stati, ad esempio Android o iOS. | varchar(255) |
...
| **`mobilepushoptin`** | Consenso push per Mobile Services | varchar(255) |
| **`mobilepushpayloadid`** | ID payload push di Mobile Services | varchar(255) |
...
| **`page_event`** | Tipo di hit inviato nella richiesta di immagine (hit standard, collegamento per il download, collegamento personalizzato, collegamento di uscita). Vedi [Ricerca eventi pagina](datafeeds-page-event.md). | tinyint senza segno |
...
| **`page_type`** | Utilizzato per popolare il [Pagine non trovate](/help/components/dimensions/pages-not-found.md) dimensione. Utilizzato esclusivamente per 404 pagine. Questa variabile deve essere vuota o contenere il valore `ErrorPage`. | char(20) |
| **`page_url`** | URL dell&#39;hit. Tieni presente che `post_page_url` viene rimosso per le richieste di immagini di tracciamento dei collegamenti e utilizza un tipo di dati varchar(255). | text |
| **`pagename`** | Utilizzato per popolare il [Pagina](/help/components/dimensions/page.md) dimensione. Se la [`pagename`](/help/implement/vars/page-vars/pagename.md) La variabile è vuota, Analytics utilizza `page_url` invece. | varchar(100) |
...
| **`persistent_cookie`** | Utilizzato in [Supporto dei cookie persistenti](/help/components/dimensions/persistent-cookie-support.md) dimensione. Indica se il visitatore supporta i cookie che non vengono scartati dopo ogni hit. | char(1) |
| **`plugins`** | Non più utilizzato. Elenco di ID numerici corrispondenti ai plug-in disponibili nel browser. Usi `plugins.tsv` ricerca. | varchar(180) |
...
| **`product_list`** | Elenco prodotti trasmesso attraverso [`products`](/help/implement/vars/page-vars/products.md) variabile. I prodotti sono delimitati da virgole, mentre le singole proprietà dei prodotti sono delimitate da punti e virgola. | text |
| **`product_merchandising`** | Non utilizzato. Utilizzo `product_list` invece. | text |
| **`prop1`** - `prop75` | Variabili di traffico personalizzate da 1 a 75. Utilizzato in [Prop](/help/components/dimensions/prop.md) dimensioni. | varchar(100) |
| **`purchaseid`** | Identificatore univoco per un acquisto, come impostato utilizzando [`purchaseID`](/help/implement/vars/page-vars/purchaseid.md) variabile. Utilizzato da `duplicate_purchase` colonna. | char(20) |
| **`quarterly_visitor`** | Flag per determinare se l’hit è un nuovo visitatore trimestrale. | tinyint senza segno |
| **`ref_domain`** | In base alla colonna referrer. Dominio di riferimento dell&#39;hit. Utilizzato in [Dominio di riferimento](/help/components/dimensions/referring-domain.md) dimensione. | varchar(100) |
| **`ref_type`** | Un ID numerico che rappresenta il tipo di riferimento per l&#39;hit. Utilizzato in [Tipo referrer](/help/components/dimensions/referrer-type.md) dimensione. <br>1: All&#39;interno del sito<br>2: Altri siti web <br>3: Motori di ricerca <br>4. Disco rigido <br>5: USENET <br>6. Digitato/Segnalibro (nessun referrer) <br>7: E-mail <br>8: Nessun JavaScript <br>9: Social network | tinyint senza segno |
| **`referrer`** | URL della pagina precedente. Utilizzato in [Referrer](/help/components/dimensions/referrer.md) dimensione. Tieni presente che `referrer` utilizza un tipo di dati varchar(255), `post_referrer` utilizza un tipo di dati varchar(244). | varchar(255) |
| **`resolution`** | ID numerico che rappresenta la risoluzione del monitoraggio. Utilizzato in [Risoluzione monitor](/help/components/dimensions/monitor-resolution.md) dimensione. Usi `resolution.tsv` tabella di ricerca. | piccolo senza segno |
...
| **`search_engine`** | ID numerico che rappresenta il motore di ricerca che ha indirizzato il visitatore al sito. Usi `search_engines.tsv` ricerca. | piccolo senza segno |
| **`search_page_num`** | Utilizzato da [Classificazione di tutte le pagine di ricerca](/help/components/dimensions/all-search-page-rank.md) dimensione. Indica in quale pagina dei risultati di ricerca il sito è stato visualizzato prima che l’utente avesse fatto clic sul sito. | piccolo senza segno |
...
| **`service`** | Non utilizzato. Utilizzo `page_event` invece. | char(2) |
...
| **`survey`** | Non più utilizzato. Variabile Adobe Survey. Solo il `post` è disponibile la colonna . | text |
...
| **`user_hash`** | Non è utile. Hash sull&#39;ID suite di rapporti. Utilizzo `username` invece. | int senza segno |
| **`user_server`** | Utilizzato in [Server](/help/components/dimensions/server.md) dimensione. | varchar(100) |
| **`userid`** | Non è utile. ID numerico dell&#39;ID suite di rapporti. Utilizzo `username` invece. | int senza segno |
| **`username`** | ID suite di rapporti per l&#39;hit. | char(40) |
| **`va_closer_detail`** | Variabile utilizzata nella variabile [Dettaglio ultimo contatto](/help/components/dimensions/last-touch-detail.md) dimensione. | varchar(255) |
| **`va_closer_id`** | ID numerico che identifica il [Canale di ultimo contatto](/help/components/dimensions/last-touch-channel.md) dimensione. La ricerca per questo ID si trova in Marketing Channel Manager. | tinyint senza segno |
| **`va_finder_detail`** | Variabile utilizzata nella variabile [Dettaglio del primo contatto](/help/components/dimensions/first-touch-detail.md) dimensione. | varchar(255) |
| **`va_finder_id`** | ID numerico che identifica il [Canale di primo contatto](/help/components/dimensions/first-touch-channel.md) dimensione. La ricerca per questo ID si trova in Marketing Channel Manager. | tinyint senza segno |
| **`va_instance_event`** | Flag per identificare il canale di marketing [Istanze](/help/components/metrics/instances.md). | tinyint senza segno |
...
| **`visit_ref_domain`** | In base ai `visit_referrer` colonna. Il primo dominio di riferimento della visita. | varchar(100) |
| **`visit_ref_type`** | ID numerico che rappresenta il tipo di referente del primo referente della visita. Utilizza il `referrer_type.tsv` tabella di ricerca. | tinyint senza segno |
| **`visit_referrer`** | Il primo referente della visita. | varchar(255) |
| **`visit_search_engine`** | ID numerico del primo motore di ricerca della visita. Usi `search_engines.tsv` ricerca. | piccolo senza segno |
...
| **`zip`** | Consente di popolare i [Codice postale](/help/components/dimensions/zip-code.md) dimensione. Consulta anche `geo_zip`. | varchar(50) |
