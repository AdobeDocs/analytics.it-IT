...
* Alcuni ID possono corrispondere a più persone e non vuoi rischiare di restituire informazioni su una persona a un&#39;altra con lo stesso ID. Ad esempio, anche se puoi verificare che il nome di una persona è Mario Rossi, non è consigliabile restituire tutti i dati su tutti i Mario Rossi nel tuo sistema.
...
* Le etichette DEL- risulteranno [anonime](/help/technotes/c-data-governance/data-labeling/gdpr-labels.md#data-governance-labels) in queste variabili (l’ID verrà sostituito da una stringa casuale con prefisso “Privacy dei dati-”). Lo stesso valore anonimizzato sostituirà tutte le istanze del valore originale in tutti i risultati identificati da un ID usato nella richiesta. Se il valore originale in questo campo è uno degli ID, le metriche del rapporto non cambieranno.
...
* Se non stai specificando l&#39;uso dell&#39;espansione dell&#39;ID, ma utilizzerai un mix di ID persona e dispositivo per richieste diverse, puoi specificare sia le etichette DEL-DEVICE che DEL-PERSON per le variabili che devono essere eliminate quando viene utilizzato uno dei due tipi di ID.
