...
| 6+ ore dopo la modifica della configurazione: aggiorna il `s.trackingServer` e `s.trackingServerSecure` variabili nel codice JavaScript di Analytics per utilizzare i nuovi server di raccolta dati. | Dopo aver apportato questa modifica, utilizza [Debugger Experience Cloud](https://experienceleague.adobe.com/docs/debugger/using/experience-cloud-debugger.html?lang=it) per verificare che la richiesta di immagine Analytics stia andando al server di raccolta dati aggiornato. |
| Immediatamente dopo l’aggiornamento del codice di Analytics: verifica sul sito che sia in corso il reindirizzamento al dominio di raccolta dati precedente. | Utilizza un [monitor pacchetto](../implement/validate/packet-monitor.md) per verificare che quando accedi al sito per la prima volta o dopo aver cancellato i cookie, vengano visualizzati due codici di stato HTTP 302 (reindirizzamento) prima del codice di stato HTTP 200 (OK). Se uno di questi reindirizzamenti non riesce, contatta immediatamente l’Assistenza clienti per assicurarti che la migrazione sia configurata correttamente. |
