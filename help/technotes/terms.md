...
* **Origini dati:** Consente a un utente di caricare dati da un file in Adobe Analytics. Il file viene in genere estratto da un sito FTP. Consulta [Origini dati](/help/import/c-data-sources/datasrc-home.md) nella guida utente Importazione.
...
* **Codice H:** Un predecessore di AppMeasurement. Nelle versioni precedenti di Adobe Analytics, le versioni dei codici venivano misurate con &quot;versione H&quot;, come H.27.5, H.26, ecc.
...
* **s.t():** Il nome della funzione in una libreria AppMeasurement che invia una richiesta di immagine per la visualizzazione della pagina. Alcune librerie AppMeasurement utilizzano `s.track()` invece. Consulta [t](../implement/vars/functions/t-method.md) nella guida utente Implementa.
* **s<span>.</span>tl():** Il nome della funzione in una libreria AppMeasurement che invia una richiesta di immagine di tracciamento dei collegamenti. Alcune librerie AppMeasurement utilizzano `s.trackLink()` invece. Consulta [tl](../implement/vars/functions/tl-method.md) nella guida utente Implementa.
