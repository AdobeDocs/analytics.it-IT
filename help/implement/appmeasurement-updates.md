...
* La distribuzione include la Libreria del servizio ID visitatore 5.5.0 e Data Integration Library 9.6.
...
* È stato migliorato AppMeasurement per rispettare la rinuncia quando il Servizio ID visitatore non è approvato in consenso. (AN-175259)
...
Questa versione assicura che la libreria AppMeasurement invii correttamente i cookie per tutti i tipi di connessione.

* AppMeasurement blocca la trasmissione dei cookie durante il POST. (AN-165538)
* Elimina il supporto per XDomainRequest. (AN-165733)
* Riduci la durata predefinita dei cookie di AppMeasurement da cinque a due anni. (AN-158572)
* Rimuovere il modulo Media da Code Manager ( AppMeasurement) (AN-166590)
...
>[!NOTE]
>
>Per i clienti che utilizzano il servizio ID visitatore è richiesto l’API Visitor 3.0 o la versione successiva. Adobe consiglia di eseguire l’aggiornamento alla versione più recente dell’API Visitor ogni volta che le librerie di codice associate vengono aggiornate (`at.js`, `AppMeasurement.js` e così via).
...
* Aggiorna AppMeasurement per utilizzare la versione aggiornata dell’interfaccia visitatore per la richiesta degli ID. (AN-151483)
...
* Aggiungi metriche di timing round trip ad AppMeasurement. (AN-158343)
...
È stato risolto un problema a causa del quale la libreria AppMeasurement non impostava sempre la combinazione di account corretta quando veniva chiamato s_gl. (AN-152153)
...
* È stato aggiunto il supporto per più ordini di creazione istanze di AppMeasurement. (AN-138237)
...
* AppMeasurement è stato aggiornato per consentire la lettura e la scrittura di cookie AMCV. (AN-127098)
* Inclusione di API Visitor 1.7.0.

>[!NOTE]
>
>Consulta anche le seguenti note sulla versione per JavaScript versione 1.6.3, che includono i requisiti aggiornati per il Servizio ID visitatore.
...
>[!IMPORTANT]
>
>La versione 1.6.0 del servizio ID visitatore *richiede* AppMeasurement per JavaScript versione 1.6.3 o successiva. Se desideri effettuare l’aggiornamento alla versione 1.6.0 del Servizio ID visitatore, assicurati di utilizzare AppMeasurement 1.6.3 o versione successiva.
...
* È stato risolto un problema che causava la chiamata da parte di AppMeasurement del metodo offuscato errato nell’API visitatore. (AN-126006)
* È stato risolto un problema che causava l&#39;errore JavaScript: &quot;Attributo valido solo in v:image&quot;. (AN-124009)
...
* È stato risolto un problema di troncamento che si verificava con l&#39;ordine delle variabili delle stringhe query in AppMeasurement, in modo che *`pageURLRest`* fosse l&#39;ultimo. (AN-114647)
...
* È stata corretta la gestione del modulo Audience Manager quando i POST vengono utilizzati per il tracciamento delle chiamate. (AN-115381)
...
* È stato aggiornato il modulo Audience Manager per l’utilizzo di Adobe Audience Manager DIL 6.2 - getCustomer ID da VisitorAPI.js, per poi trasmetterli nella chiamata /event a Adobe Audience Manager. (AN-104978)
...
* Supporto HTTP POST.
  ...
  >[!IMPORTANT]
  >
  >Affinché una chiamata Analytics possa utilizzare il metodo `POST` invece del metodo `GET` in AppMeasurement (un metodo per risolvere [URL troncati in IE](/help/implement/js/troubleshooting.md)), è necessario utilizzare l&#39;implementazione più recente del servizio ID visitatori per CX Enterprise.
...
* È stata corretta la gestione dei flag di attesa e di fine per i campi dell’API Visitor, come l’ID visitatore Analytics legacy, che causava errori.
* Supporto per le nuove funzioni del servizio ID visitatore 1.3.

## Versione 1.3.1

Data di rilascio: **22 maggio 2014**

* La funzione di AppMeasurement per JavaScript `s_gi` non stava trovando correttamente le istanze create utilizzando il codice H `s_gi`. Questo problema ha interessato solo alcune implementazioni di doppio tag in cui AppMeasurement for JavaScript e il codice H si trovavano sulla stessa pagina con istanze separate e `s_gi` veniva utilizzato per trovare le istanze per suite di rapporti.
...
* È stato risolto un problema di compatibilità con il modulo DIL di Audience Manager. I clienti di Audience Manager devono anche eseguire l’aggiornamento alla versione 4.8 del modulo DIL.
